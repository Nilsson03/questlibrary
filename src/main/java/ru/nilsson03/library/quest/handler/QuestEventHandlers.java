    ...
    protected void registerHandlers() {
        ...
        QuestEventHandler<EntityExplodeEvent> tntBreakBlocksHandler = (event) -> {
            if (event.getEntity() != null && event.getEntity().getType() == EntityType.PRIMED_TNT) {
                ...
                if (blockCount > 0 && tnt.getPersistentDataContainer().has(TNT_IGNITER_KEY, PersistentDataType.STRING)) {
                    String playerUuidString = tnt.getPersistentDataContainer().get(TNT_IGNITER_KEY, PersistentDataType.STRING);
        ...
        // Регистрируем вспомогательный обработчик для отслеживания игрока при трансформации зомби-жителя
